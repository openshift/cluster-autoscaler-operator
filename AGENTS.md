...
This repo is the **Cluster Autoscaler Operator** for OpenShift — a standalone operator (`github.com/openshift/cluster-autoscaler-operator`) that manages the lifecycle of the Kubernetes Cluster Autoscaler on OpenShift clusters. It does not contain the autoscaler itself; instead, it creates and manages a Deployment that runs the upstream autoscaler image.
...
- The upstream autoscaler lives in the [openshift/kubernetes-autoscaler](https://github.com/openshift/kubernetes-autoscaler) fork (a downstream fork of `kubernetes/autoscaler`).
...
- Upstream autoscaler: [openshift/kubernetes-autoscaler](https://github.com/openshift/kubernetes-autoscaler)
